...
double x = if (dbl > 9) then (dbl - 9) else dbl 
  where dbl = x + x


isValid :: String -> Bool
isValid n | test1 = test2
          | otherwise = False
  
  where test1 = length (filter p n) == 0
          where p = (\x -> (not . isDigit) x && x /= ' ') 
          
        test2 | length n' <= 1 = False 
              | otherwise = digitSum `mod` 10 == 0   
      
          where n' = filter isDigit n
                digitSum = sum (altMap id double digits) 
                digits = reverse $ map digitToInt n'
